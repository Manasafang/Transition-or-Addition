...
- ภาพที่ 8: หน่วย TWh ฝั่งซ้ายของแต่ละกราฟคือการเปลี่ยนแปลงของเชื้อเพลิงฟอสซิล ฝั่งขวาคือหมุนเวียนที่เพิ่ม เรียงตามหมุนเวียนที่เพิ่ม สองฝั่งของแต่ละกราฟใช้สเกลเดียวกัน แต่สองกราฟใช้สเกลต่างกัน จึงไม่ควรเทียบความยาวแท่งข้ามกราฟ แท่งที่มีรอยขาดถูกตัดที่ 1,300 TWh (ตัวเลขข้างแท่งคือค่าจริง)
...
---
<a id="rq2"></a>
## ตรวจสอบสมมติฐาน: ความต้องการไฟฟ้าที่เติบโต vs. การเติบโตของพลังงานหมุนเวียน (Electricity Demand vs. Renewable Growth)

🌟 ส่วนที่ผ่านมาแสดงว่ามีทั้งประเทศที่เป็น Transition และ Addition คำถามถัดไปคือ **สองกลุ่มนี้ต่างกันตรงไหน** ส่วนนี้ทดสอบว่าอะไรแยกสองกลุ่ม ไม่ได้ศึกษาสาเหตุของการเติบโตของความต้องการ

> **คำถาม (RQ2):** ความต้องการไฟฟ้าที่โตมาก หรือการสร้างพลังงานหมุนเวียนที่ช้า อย่างไหนสัมพันธ์กับการเป็น Addition มากกว่า?
>
...
**วัดอย่างไร:** วัดทั้งสองอย่างเป็น % ของไฟฟ้าที่ผลิตปี 2000 ช่วงปี 2000 → 2024
...
ทั้งสองตัวหารด้วยฐานเดียวกันเพื่อให้เทียบข้ามประเทศขนาดต่างกันได้ ใช้ 104 ประเทศที่ผลิตไฟฟ้าเกิน 5 TWh ในปี 2000 (เหตุผลของเกณฑ์นี้อยู่ในหัวข้อตรวจความทนทานด้านล่าง) แบ่งเป็น Transition 37 ประเทศ, Addition 63 ประเทศ และกลุ่มอื่น 4 ประเทศ (Fossil Expansion 3, Decline 1) ซึ่งไม่นำมาเทียบ

> 📑 **หมายเหตุ:** ในโปรเจกต์นี้ใช้ปริมาณไฟฟ้าที่ผลิตแทนความต้องการไฟฟ้า จึงไม่รวมผลของการนำเข้า-ส่งออกไฟฟ้าและการสูญเสียในระบบ

---
**Figure 9: ความต้องการโตเทียบกับหมุนเวียนที่เพิ่ม (Demand Growth vs Renewable Growth, 104 Countries)**

![Figure 9](images/figure9.png)

- ภาพที่ 9: ความต้องการไฟฟ้าที่โต (แกน X) เทียบกับหมุนเวียนที่เพิ่ม (แกน Y) ของ 104 ประเทศ ปี 2000 → 2024 ทั้งสองแกนเป็น % ของไฟฟ้าที่ผลิตปี 2000 และใช้สเกลเดียวกัน (symlog: ยืดค่าน้อย ย่อค่ามาก เพื่อแสดงประเทศที่ความต้องการลดลงได้) เส้นประคือหมุนเวียนที่เพิ่มเท่ากับไฟฟ้าใหม่พอดี (เส้นแบ่ง Transition โดยประมาณ) เหนือเส้น = หมุนเวียนโตเกินความต้องการ วงกลมเขียว = Transition สามเหลี่ยมเหลือง = Addition สี่เหลี่ยมเทา = กลุ่มอื่น จุดใหญ่คือ 6 ประเทศตัวอย่าง สามเหลี่ยมบนแกนคือมัธยฐานของแต่ละกลุ่ม (ผลลัพธ์ชี้ว่าความต้องการต่างกันมากระหว่างสองกลุ่ม แต่หมุนเวียนที่เพิ่มซ้อนทับกัน)

**ข้อสรุปสำคัญ (Key Takeaways):** ประเทศ Transition (สีเขียว) กระจายอยู่ด้านซ้ายและเหนือเส้นประ ความต้องการนิ่งหรือลดลง (มัธยฐาน +5.5%) ส่วนประเทศ Addition (สีเหลือง) กระจายอยู่ด้านขวาและใต้เส้นประ ความต้องการโตมาก (มัธยฐาน +144%) ขณะที่ความสูงของสองกลุ่มซ้อนกันชัดเจน (หมุนเวียนที่เพิ่ม +31% เทียบ +26%)

💡 ผลลัพธ์นี้แสดงว่า **ประเทศ Addition ไม่ได้สร้างหมุนเวียนช้ากว่า** แต่ความต้องการโตเกินกว่าหมุนเวียนจะตามทัน

<details>
<summary>ตารางที่ 5: เปรียบเทียบสองกลุ่ม และคำอธิบายค่า "แยกกลุ่มได้" 0.95 / 0.50</summary>

**Table 5: เปรียบเทียบสองกลุ่ม (Median Comparison)**
...
*"แยกกลุ่มได้" บอกว่าตัวเลขนั้นแยกสองกลุ่มได้ดีแค่ไหน อ่านได้ว่า **สุ่มประเทศ Addition หนึ่งประเทศ กับ Transition หนึ่งประเทศ ความน่าจะเป็นที่ประเทศ Addition จะมีค่าสูงกว่าเป็นเท่าไร**

- **0.50** = ไม่ต่างกัน (เหมือนโยนเหรียญ)
- **1.00** = แยกได้สมบูรณ์ (Addition สูงกว่าทุกประเทศ)
- **0.00** = แยกได้สมบูรณ์ในทางกลับกัน

ความต้องการ 0.95 แปลว่าประเทศ Addition มีความต้องการโตมากกว่าประเทศ Transition ในราว 95% ของคู่ที่สุ่มมา (p = 6.9×10⁻¹⁴) ส่วนหมุนเวียน 0.50 ไม่มีความต่างที่ตรวจพบ (p = 0.98) คำนวณด้วยการทดสอบ Mann–Whitney U (ค่า = U ÷ (n₁ × n₂)) ซึ่งใช้อันดับของข้อมูล จึงไม่ไวต่อค่าสุดขั้วอย่างเวียดนาม (+1,043%) ที่มาจากฐานปี 2000 ที่เล็ก

</details>

---
**Figure 10: ไฟฟ้าใหม่ถูกรองรับด้วยอะไร (Where New Generation Came From, 6 Countries)**

![Figure 10](images/figure10.png)

- ภาพที่ 10: การแยกส่วนของไฟฟ้าที่เพิ่มขึ้นใหม่ว่าถูกรองรับด้วยแหล่งใด 6 ประเทศ ปี 2000 → 2024 หน่วย TWh แต่ละกราฟย่อยใช้สเกลแนวตั้งของตัวเอง จึงเปรียบเทียบความสูงข้ามประเทศไม่ได้ ให้อ่านจากตัวเลข อ่านจากซ้ายไปขวา: แท่งเทา = การเปลี่ยนแปลงของไฟฟ้าที่ผลิตรวม (Δ ความต้องการ) แท่งเขียว = ส่วนที่หมุนเวียนรองรับ แท่งม่วง = Nuclear offset (+ คือนิวเคลียร์ลดลง − คือนิวเคลียร์เพิ่มขึ้น) แท่งส้ม = ส่วนที่เหลือให้เชื้อเพลิงฟอสซิล (Δ ฟอสซิล) แท่งเขียวและแท่งม่วงเป็นส่วนที่ "หักออก" จากความต้องการ ค่าที่แสดงจึงเป็นค่าลบของการเปลี่ยนแปลงจริง (Nuclear offset = −Δ นิวเคลียร์) สมการที่ใช้: Δ ฟอสซิล = Δ ความต้องการ − Δ หมุนเวียน − Δ นิวเคลียร์ เวียดนามและไทยไม่มีไฟฟ้านิวเคลียร์ จึงไม่แสดงแท่งม่วง

**ข้อสรุปสำคัญ (Key Takeaways):** 6 ประเทศใช้สมการเดียวกัน แต่ที่มาของผลลัพธ์ต่างกัน 3 แบบ
...
> 📑 **หมายเหตุ:** การจำแนกกลุ่มยังใช้เฉพาะหมุนเวียนและฟอสซิลตามนิยามในตารางที่ 1 นิวเคลียร์แสดงเป็นปัจจัยอธิบายเท่านั้น ผลกระทบต่อการจัดกลุ่มดูในหัวข้อตรวจความทนทานถัดไป

---
**ตรวจความทนทาน (Robustness Checks):** ทดสอบสองเรื่อง

**(ก) ถ้านับนิวเคลียร์เป็นพลังงานสะอาดด้วย** ใช้ Δ (หมุนเวียน + นิวเคลียร์) แทน Δ หมุนเวียน ด้วยเกณฑ์ 4 กลุ่มเดิม กับ 33 ประเทศที่มีนิวเคลียร์ในปี 2000 หรือ 2024 พบว่า **30 จาก 33 ประเทศไม่เปลี่ยนกลุ่ม** ญี่ปุ่นเป็นกรณีเดียวที่เปลี่ยนอย่างมีนัย (นิวเคลียร์ลดลง 234 TWh มากกว่าหมุนเวียนที่เพิ่ม 132 TWh) ไต้หวันและลิทัวเนียเปลี่ยนเพราะไฟฟ้าสะอาดรวมใกล้ศูนย์ และไม่มีประเทศใดเปลี่ยนไปทางที่ดีขึ้น

<details>
<summary>ตารางที่ 6: รายละเอียดและตารางเต็ม 33 ประเทศ (ทดสอบนิวเคลียร์)</summary>

การเปลี่ยนกลุ่มเมื่อนับนิวเคลียร์: Transition 18 → 17, Addition 15 → 13, Fossil Expansion 0 → 2, Decline 0 → 1 ในหัวข้อนี้ใช้ Δ นิวเคลียร์ (ค่าการเปลี่ยนแปลงจริง) ตลอด

- **ญี่ปุ่น** นิวเคลียร์ลดลง 234 TWh มากกว่าหมุนเวียนที่เพิ่ม (+132 TWh) ไฟฟ้าสะอาดรวมจึงลดลง ขณะที่ฟอสซิลเพิ่มขึ้น
...
- **จีน รัสเซีย เกาหลีใต้** เพิ่มนิวเคลียร์มาก (+434, +85, +80 TWh) ฟอสซิลก็ยังเพิ่มขึ้นมากกว่าอยู่ดี จึงยังเป็น Addition
- **เยอรมนี** ยังเป็น Transition แต่ส่วนเพิ่มของไฟฟ้าสะอาดลดลงจาก +255 เหลือ +86 TWh เพราะปิดนิวเคลียร์ทั้งหมด

**Table 6: ผลการจำแนกกลุ่มเมื่อนับนิวเคลียร์เป็นพลังงานสะอาด (Classification with Nuclear Counted as Clean)**
...
**(ข) ถ้าเปลี่ยนเกณฑ์ขั้นต่ำ ขนาดระบบ หรือการมีนิวเคลียร์** ทดสอบซ้ำการเปรียบเทียบในตารางที่ 5 พบว่า **ความต้องการแยกสองกลุ่มได้ทุกเงื่อนไข** (0.88–0.96 โดยขอบล่างของช่วงเชื่อมั่นไม่ต่ำกว่า 0.79) ส่วนหมุนเวียนที่เพิ่มไม่ต่างกันทุกเงื่อนไข (0.41–0.50 ช่วงเชื่อมั่นคร่อม 0.50 ทุกแถว) แม้ในระบบใหญ่ประเทศ Addition จะต่ำกว่าเล็กน้อย (มัธยฐานราว +19–20% เทียบ +31%) แต่ไม่ชัดทางสถิติ

<details>
<summary>ตารางที่ 7: ทำไมเลือกเฉพาะประเทศที่ผลิตไฟฟ้าเกิน 5 TWh ในปี 2000 และผลเมื่อเปลี่ยนเงื่อนไข</summary>

ตัววัดของส่วนนี้หารด้วยไฟฟ้าที่ผลิตปี 2000 ประเทศที่ฐานเล็กมากจึงได้เปอร์เซ็นต์สูงเกินจริง (เช่น เอธิโอเปียจาก 1.7 เป็น 25.2 TWh คือ +1,391%) และการจัดกลุ่มก็เปราะบาง เพราะฟอสซิลตั้งต้นเล็กจิ๋ว (เอธิโอเปีย 0.02 TWh) แค่หายไปก็นับเป็น Transition ทั้งที่ความต้องการพุ่ง ในประเทศที่ผลิตไม่เกิน 5 TWh มี Transition 18 ประเทศ และ 7 ประเทศมีความต้องการโตเกิน 100% ขณะที่ในประเทศ Transition ที่เกิน 5 TWh ไม่มีประเทศใดโตเกิน 100% จึงตัดประเทศที่ฐานไม่เกิน 5 TWh ออก เหลือ 104 ประเทศ (ตัดแถวที่ไม่ใช่ประเทศ เช่น World ทวีป และกลุ่มรายได้ ด้วย `iso_code` ก่อน) เกณฑ์นี้เป็นค่าที่เลือกเอง และสรุปไม่เปลี่ยนเมื่อเปลี่ยนเกณฑ์

**Table 7: เปรียบเทียบ Transition (T) กับ Addition (A) เมื่อเปลี่ยนเงื่อนไข (Robustness Across Conditions)**

| เงื่อนไข (ผลิตปี 2000 เกิน) | ประเทศทั้งหมด | T / A | ความต้องการโต (มัธยฐาน T / A) | แยกกลุ่มได้ (ช่วงเชื่อมั่น 95%) | หมุนเวียนที่เพิ่ม (มัธยฐาน T / A) | แยกกลุ่มได้ (ช่วงเชื่อมั่น 95%) |
|---|---|---|---|---|---|---|
| ไม่ตัด | 192 | 55 / 124 | +13.9% / +155.3% | 0.88 (0.81–0.93) | +35.8% / +30.6% | 0.49 (0.40–0.58) |
| 0.5 TWh | 149 | 48 / 94 | +11.3% / +137.0% | 0.89 (0.82–0.95) | +32.9% / +26.6% | 0.48 (0.39–0.58) |
| 1 TWh | 139 | 48 / 87 | +11.3% / +150.3% | 0.89 (0.82–0.96) | +32.9% / +27.8% | 0.49 (0.39–0.59) |
| 2 TWh | 122 | 41 / 77 | +8.0% / +144.4% | 0.92 (0.87–0.97) | +33.3% / +27.8% | 0.49 (0.38–0.59) |
| **5 TWh (ที่ใช้)** | **104** | **37 / 63** | **+5.5% / +144.4%** | **0.95 (0.90–0.98)** | **+31.1% / +25.9%** | **0.50 (0.39–0.61)** |
| 10 TWh | 81 | 31 / 47 | +7.8% / +115.5% | 0.95 (0.90–0.99) | +31.1% / +19.5% | 0.42 (0.30–0.56) |
| 20 TWh | 63 | 27 / 34 | +7.8% / +115.4% | 0.94 (0.86–0.99) | +31.1% / +18.5% | 0.41 (0.26–0.57) |
| 50 TWh | 40 | 19 / 20 | +7.8% / +122.4% | 0.91 (0.79–1.00) | +31.1% / +19.9% | 0.46 (0.25–0.67) |
| 5 TWh และไม่มีนิวเคลียร์ | 71 | 19 / 48 | +7.8% / +153.3% | 0.96 (0.90–0.99) | +37.5% / +26.1% | 0.50 (0.36–0.65) |
| 5 TWh และมีนิวเคลียร์ | 33 | 18 / 15 | +5.5% / +75.6% | 0.92 (0.80–1.00) | +24.0% / +19.5% | 0.46 (0.25–0.69) |

"แยกกลุ่มได้" คือความน่าจะเป็นที่ประเทศ Addition ที่สุ่มมามีค่าสูงกว่าประเทศ Transition (0.50 = ไม่ต่างกัน ดูคำอธิบายใต้ตารางที่ 5) ช่วงเชื่อมั่น 95% คำนวณด้วย bootstrap (สุ่มประเทศซ้ำ 2,000 รอบ) "ประเทศทั้งหมด" รวมกลุ่มอื่น (Fossil Expansion, Decline) ที่ไม่นำมาเทียบ "มีนิวเคลียร์" คือมีไฟฟ้านิวเคลียร์ในปี 2000 หรือ 2024

**อ่านผล:** ความต้องการแยกสองกลุ่มได้ทุกแถว ค่าเพิ่มจาก 0.88 (ไม่ตัด) เป็น 0.95 (5 TWh) เพราะประเทศ Transition ฐานเล็กบางประเทศมีความต้องการโตมาก แต่ผลต่างนี้เล็ก (ทดสอบผลต่างโดยตรง +0.07 ช่วงเชื่อมั่น 95% +0.01 ถึง +0.14) ส่วนหมุนเวียนที่เพิ่มไม่ต่างกันทุกแถว เพราะช่วงเชื่อมั่นคร่อม 0.50 ทั้งหมด ในระบบใหญ่ (>20 และ >50 TWh) ประเทศ Addition ต่ำกว่าเล็กน้อย แต่ไม่ชัดทางสถิติ (p = 0.22 และ 0.66 จากประเทศ 19–34 ประเทศต่อกลุ่ม)

</details>

---
**ข้อสรุปสำคัญ (Key Takeaways):**  ประเทศฝั่ง Addition ไม่ได้สร้างพลังงานหมุนเวียนช้ากว่า แต่ความต้องการไฟฟ้าโตมากกว่ามากจนพลังงานหมุนเวียนตามไม่ทัน ในทุกเงื่อนไขที่ทดสอบ ความต้องการที่โตเป็นตัวแยก Transition ออกจาก Addition ส่วนความเร็วในการสร้างพลังงานหมุนเวียนไม่ต่างกันอย่างชัดเจน

> 📑 **ข้อควรระวัง:**
>
> - **เป็นความสัมพันธ์ ไม่ใช่การพิสูจน์เหตุและผล** ผลนี้บอกว่าสองอย่างเกิดไปด้วยกัน
> - **ฝั่งความต้องการมีส่วนที่เป็นเรื่องนิยาม** เพราะการจัดกลุ่มเทียบพลังงานหมุนเวียนกับเชื้อเพลิงฟอสซิลซึ่งเชื่อมกับความต้องการตามสมการบัญชีอยู่แล้ว ส่วนที่บอกข้อมูลใหม่จริงคือ **พลังงานหมุนเวียนที่เพิ่มไม่ต่างกันอย่างชัดเจน**
> - **เส้นประในภาพที่ 9 เป็นเส้นแบ่งโดยประมาณ** เพราะไม่คิดนิวเคลียร์ ญี่ปุ่น (Addition) อยู่เหนือเส้นเพราะนิวเคลียร์ลดลง ส่วนสโลวีเนีย (Transition) อยู่ใต้เส้นเพียงนิดเดียว (+22.8% เทียบ +21.8%) เพราะนิวเคลียร์เพิ่มเล็กน้อย
